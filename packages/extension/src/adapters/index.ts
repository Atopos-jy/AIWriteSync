...
import {
  ...
  CSDNAdapter,
  ToutiaoAdapter,
  ToutiaoExtractAdapter,
  BaijiahaoExtractAdapter,
  BilibiliExtractAdapter,
  WeiboAdapter,
  BilibiliAdapter,
  BaijiahaoAdapter,
  YuqueAdapter,
...
const adapterEntries: AdapterEntry[] = ADAPTER_CLASSES.map((AdapterClass) => {
  ...
  return {
    ...
    preprocessConfig: (
      instance as unknown as { preprocessConfig?: Record<string, unknown> }
    ).preprocessConfig,
  };
});
...
export async function checkAllPlatformsAuth(forceRefresh = false) {
  ...
  for (const meta of metas) {
    ...
    const cacheValid =
      cached && now - cached.timestamp < cacheTTL && !forceRefresh;

    if (cacheValid) {
      logger.debug(
        ` Using cached auth for ${meta.id} (TTL: ${cacheTTL / 1000}s)`,
      );
      results.push({
        ...meta,
...
export async function syncToPlatform(
  ...
  try {
    ...
    const platformContents = (article as any).platformContents as
      | Record<string, { html: string; markdown: string }>
      | undefined;
      ...
      adapter.publish(platformArticle, {
        ...
        onImageProgress: onImageProgress
          ? (current: number, total: number) =>
              onImageProgress(platformId, current, total)
          : undefined,
      }),
      PUBLISH_TIMEOUT,
...
export async function syncToMultiplePlatforms(
  ...
  // 调试日志：记录同步开始信息
  console.log(`[Sync] 开始同步到 ${platformIds.length} 个平台:`, platformIds);
  console.log(`[Sync] 文章信息:`, {
    title: article.title,
    ...
    url: article.url,
    source: article.source?.platform,
  });
  ...
  const syncOne = async (platformId: string): Promise<SyncResult> => {
    ...
    const wrappedImageProgress: ImageProgressCallback | undefined =
      callbacks?.onImageProgress || callbacks?.onDetailProgress
  ...
  // 调试日志：记录同步完成信息
  console.log(`[Sync] 同步完成，耗时 ${duration}ms`, {
